  ...
  override def parse(bytes: Array[Byte], rowCount: Int): Array[Byte] = {
    ...
    dataType match {
      case BinaryType | StringType =>
        ...
        valuesReader.initFromPage(rowCount, ByteBuffer.wrap(bytes), bitsDataLength + 4)
...
private[oap] case class PlainDictionaryFiberParser(
  ...
  override def parse(bytes: Array[Byte], rowCount: Int): Array[Byte] = {
    ...
    valuesReader.initFromPage(rowCount, ByteBuffer.wrap(bytes), bits.toLongArray().length * 8 + 4)
