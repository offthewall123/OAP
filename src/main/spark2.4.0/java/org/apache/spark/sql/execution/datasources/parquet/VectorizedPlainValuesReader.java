...
public class VectorizedPlainValuesReader extends ValuesReader implements VectorizedValuesReader {
  ...
  private ByteBuffer getBuffer(int length) {
    try {
      if (this.byteBuffer.remaining() < length) {
        ...
        ByteBuffer copy = this.byteBuffer.duplicate();
        copy.limit(copy.position() + length);
        this.byteBuffer.position(this.byteBuffer.position() + length);
        return copy.order(ByteOrder.LITTLE_ENDIAN);
      }
    } catch (IOException var3) {
      throw new ParquetDecodingException("Failed to read " + length + " bytes", var3);
    }
  }
  ...
  public final boolean readBoolean() {
    ...
    if (bitOffset == 0) {
      currentByte = Platform.getByte(byteBuffer, offset);
    }
